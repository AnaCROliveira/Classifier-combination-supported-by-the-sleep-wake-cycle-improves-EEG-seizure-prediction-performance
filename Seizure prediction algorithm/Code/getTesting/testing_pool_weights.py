...
def main_test_pool_weights(patient, data_list, times_list, metadata_list, vigilance_list, SPH, window_size):
    
    print('\nTesting...')

    # --- Load models ---  
    models_awake = pickle.load(open(f'../Results/Models/Pool_weights/model_awake_patient{patient}', 'rb'))
    models_sleep = pickle.load(open(f'../Results/Models/Pool_weights/model_sleep_patient{patient}', 'rb'))  
    ...
    for SOP in SOP_list:
        ...
        for seizure in range(n_seizures):
            ...
            # --- Figure: test ---
            fig_test(patient, seizure, SOP, times, target, prediction, firing_power, firing_power_threshold, alarm, vigilance, 'Pool_weights')
            fig_temporality(patient, seizure, SOP, SPH, times, onset_list[seizure], target, prediction, firing_power, firing_power_threshold, alarm, vigilance, window_size, 'Pool_weights')
