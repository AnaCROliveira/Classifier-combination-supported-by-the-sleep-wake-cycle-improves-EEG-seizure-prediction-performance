...
def main_test_control(patient, data_list, times_list, metadata_list, vigilance_list, SPH, window_size):
    
    print('\nTesting...')

    # --- Load models ---  
    models = pickle.load(open(f'../Results/Models/Control/model_patient{patient}', 'rb'))  
    ...
    for SOP in SOP_list:
        ...
        for seizure in range(n_seizures):
            ...
            # --- Figure: test ---
            fig_test(patient, seizure, SOP, times, target, prediction, firing_power, firing_power_threshold, alarm, vigilance, 'Control')
            fig_temporality(patient, seizure, SOP, SPH, times, onset_list[seizure], target, prediction, firing_power, firing_power_threshold, alarm, vigilance, window_size, 'Control')
