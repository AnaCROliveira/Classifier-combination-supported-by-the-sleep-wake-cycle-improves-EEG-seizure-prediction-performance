...
def main_test_feature_state(patient, data_list, times_list, metadata_list, vigilance_list, SPH, window_size):
    
    print('\nTesting...')

    # --- Load models ---  
    models = pickle.load(open(f'../Results/Models/Feature_state/model_patient{patient}', 'rb'))  
    ...
    # --- Add vigilance as an extra feature ---
    data_list = [np.column_stack([data_list[seizure], vigilance_list[seizure]]) for seizure in range(n_seizures)]
    ...
    for SOP in SOP_list:
        ...
        for seizure in range(n_seizures):
            ...
            # --- Figure: test ---
            fig_test(patient, seizure, SOP, times, target, prediction, firing_power, firing_power_threshold, alarm, vigilance, 'Feature_state')
            fig_temporality(patient, seizure, SOP, SPH, times, onset_list[seizure], target, prediction, firing_power, firing_power_threshold, alarm, vigilance, window_size, 'Feature_state')
