...
from regularization import get_firing_power, alarm_generation_2thresholds_transitions, alarm_processing
...
def main_test_threshold_transitions(patient, data_list, times_list, metadata_list, vigilance_list, SPH, window_size):
    
    print('\nTesting...')

    # --- Load models ---  
    models = pickle.load(open(f'../Results/Models/Threshold_transitions/model_patient{patient}', 'rb'))  
    ...
    firing_power_threshold_transition = 0.25
    ...
    for SOP in SOP_list:
        ...
        for seizure in range(n_seizures):
            ...
            alarm, threshold = alarm_generation_2thresholds_transitions(firing_power, firing_power_threshold, firing_power_threshold_transition, times, vigilance)            
            ...
            # --- Figure: test ---
            fig_test(patient, seizure, SOP, times, target, prediction, firing_power, threshold, alarm, vigilance, 'Threshold_transitions')
            fig_temporality(patient, seizure, SOP, SPH, times, onset_list[seizure], target, prediction, firing_power, threshold, alarm, vigilance, window_size, 'Threshold_transitions')
        ...
        info_test.append([ss_samples, sp_samples, firing_power_threshold_transition, true_alarm, false_alarm, ss, FPRh, surr_ss_mean, surr_ss_std, tt, pvalue])
